...
function populate(model, docs, options, cb) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (mod.options.options && mod.options.options.skip) {
        assignmentOpts.skipLimit = mod.options.options.skip;
    }else{
    	assignmentOpts.skipLimit = 0;
    }
...
function valueFilter(val, assignmentOpts) {
  if (Array.isArray(val)) {
    ...
    for (var i = assignmentOpts.skipLimit; i < numValues; ++i) {
      var subdoc = val[i];
